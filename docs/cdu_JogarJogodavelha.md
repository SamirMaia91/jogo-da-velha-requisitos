...
* **A1.7.** Caso o formato selecionado seja **Melhor de 3 (MD3)**:
  ...
  * **A1.7.2.** Caso nenhum jogador tenha atingido 2 vitórias e a rodada seja menor que 3, o sistema incrementa o número da rodada, aguarda 2 segundos, limpa o tabuleiro, esconde a linha vitoriosa e inicia a nova rodada mantendo o placar acumulado e definindo o Jogador X como o primeiro a jogar.
...
| **UI-06** | Contador de Rodada         | Texto                         | Formato`Atual/Total` (ex: `1/1` ou `1/3`)           | `1/1`                  | Exibe`1/1` em Partida Única, atualiza até `3/3` no modo Melhor de 3 e mantém o valor atual quando há empate.   |
...
| **UI-08** | Status do Jogo             | Texto                         | Mensagem dinâmica (ex:`Vez do Jogador X`, `Rodada Empatada!`) | `Vez do Jogador X`     | Atualiza a cada jogada ou término de rodada.                                 |
...
| `modeSelectValue`     | String                          | `'pvp'`                 | Configuração do modo de jogo (`'pvp'` ou `'cpu'`).                         |
| `formatSelectValue`   | String                          | `'single'`              | Configuração do formato da disputa (`'single'` ou `'bo3'`).                |
| `roundFinished`       | Boolean                         | `false`                 | Indica que a rodada terminou e impede novas jogadas durante a transição.      |
...
> A marcação `[x]` indica cobertura verificada por inspeção da implementação e pelo roteiro reproduzível registrado em [`EVIDENCIAS_TESTES.md`](./EVIDENCIAS_TESTES.md).
